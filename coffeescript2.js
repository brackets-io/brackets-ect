    ...
    CodeMirror.defineMode("coffeescript2", function (conf, parserConf) {
        ...
        function tokenBase(stream, state) {
            ...
            if (stream.sol()) {
                ...
                if (stream.eatSpace()) {
                    ...
                    if (lineOffset > scopeOffset && state.scope.type == "coffee2") {
                        return "indent";
        ...
        function indent(stream, state, type) {
            ...
            for (var scope = state.scope; scope; scope = scope.prev) {
                if (scope.type === "coffee2" || scope.type == "}") {
        ...
        function tokenLexer(stream, state) {
            ...
            if (delimiter_index !== -1) {
                while (state.scope.type == "coffee2" && state.scope.prev)
                    state.scope = state.scope.prev;
                if (state.scope.type == current)
                    state.scope = state.scope.prev;
            }
            if (state.dedent && stream.eol()) {
                if (state.scope.type == "coffee2" && state.scope.prev)
                    state.scope = state.scope.prev;
                state.dedent = false;
            }
        ...
        var external = {
            ...
            indent: function (state, text) {
                ...
                if (closer)
                    while (scope.type == "coffee2" && scope.prev) scope = scope.prev;
